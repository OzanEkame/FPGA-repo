...
Constraints: constraints/board.xdc
 — define 100 MHz CLK and I/O pin map

Build → Generate bitstream → Program device

Play: the display shows a number; use the switches to input its binary equivalent and press OK
...
File	Description
rtl/TopLevelGame.v
	Main game FSM and module integration
rtl/LFSR.v
	16-bit pseudo-random number generator
rtl/Debounce.v
	Two-flip-flop synchronizer + integrator
rtl/BinaryToBCD_12bit.v
	Binary-to-BCD conversion (double-dabble)
rtl/DisplayMultiplexer.v
	7-segment display multiplexer
rtl/SevenSegmentDecoder.v
	Binary-to-7-segment code encoder

Each module is individually verified through simulation.

Simulation Testbenches
Testbench	Purpose
sim/TopLevelGame_tb.v
	Full game FSM and integration test
sim/Debounce_tb.v
	Input debounce verification
sim/BinaryToBCD_12bit_tb.v
	Binary→BCD conversion validation
sim/DisplayMultiplexer_tb.v
	Refresh rate and digit cycle testing
sim/SevenSegmentDecoder_tb.v
	Segment encoding validation
